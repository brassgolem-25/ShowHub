...
export class LiveEventController{
    static  insertLiveEventsDetails = async (req, res) => {
        try {
    ...
    static  insertVenueDetails = async (req, res) => {
        try {
            const requestData = req.body.data;
            for (let obj of requestData) {
    ...
    static  insertTicketDetails = async (req, res) => {
        try {
            const requestData = req.body.data;
            for (let obj of requestData) {
    ...
    static  getBasicLiveEventsByLocation = async (req, res) => {
        try {
            const { city } = req.body;
    ...
    static  getLiveEventsByEventCode = async (req, res) => {
        try {
    ...
    static  generateJSONData = async (req, res) => {
        try {
            ...
            const parseDates = (startDate, whenDetails) => {
                ...
                const parsedStartDate = this.parseDate(new Date(startDate + " " + year));
                let endDate = whenDetails.split(" – ")[1];
                if (!endDate) {
                    endDate = parsedStartDate;
                } else {
                    const [day, rest] = endDate.split(", ");
                    endDate = this.parseDate(new Date(`${day} ${rest} ${parsedStartDate.slice(6)}`));
                }
